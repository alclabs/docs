...
public class MissingDocumentCheckTest extends Specification
{
   ...
   def "test performCheckOnRef with missing file"()
   {
      ...
      then:
         ...
         result.getText() == 'Missing document.  Referenced from "#foo/bar", file "/not/a/real/path" could not be found (looked for it at "'+new File("/install/webroot/system/webapp_data/addonname/private/content/not/a/real/path").absolutePath+'"'
   }
   ...
   def "test performCheckOnRef with real file"()
   {
      given:
         ...
         publicDir.mkdirs()
         File privateDocsDir = new File(tempDir, "webroot/system/webapp_data/addonname/private/content")
